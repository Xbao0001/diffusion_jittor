...
class GaussianDiffusion(nn.Module):
    ...
    def register_schedule(self, beta_schedule, timesteps):
        ...
        self.register_buffer('posterior_log_variance_clipped', to_var(
            np.log(np.clip(posterior_variance, a_min=1e-20))))
        self.register_buffer('posterior_mean_coef1', to_var(
            betas * np.sqrt(alphas_cumprod_prev) / (1. - alphas_cumprod)))
        self.register_buffer('posterior_mean_coef2', to_var(
            (1. - alphas_cumprod_prev) * np.sqrt(alphas) / (1. - alphas_cumprod)))
        self.register_buffer('p2_loss_weight', (self.p2_loss_weight_k +
                             alphas_cumprod / (1 - alphas_cumprod)) ** -self.p2_loss_weight_gamma)
    ...
    @jt.no_grad()
    def p_sample_loop(self, shape, clip_denoised=None):
...
#         x = self.encode_first_stage(x).detach()  # B 3 H W -> B 6 H//4 W//4
