...
def get_timestep_embedding(timesteps: jt.Var, embedding_dim: int):
    """
    ...
    emb = jt.exp(jt.arange(start=0, end=half_dim, dtype=jt.float32) * -emb)
    # emb = emb.to(device=timesteps.device)
    emb = timesteps.float32()[:, None] * emb[None, :]
    emb = jt.concat([jt.sin(emb), jt.cos(emb)], dim=1)
    if embedding_dim % 2 == 1:  # zero pad
        raise ValueError('dim should be even')
        # emb = nn.pad(emb, (0,1,0,0))
    return emb
...
class AttnBlock(nn.Module):
    ...
    def execute(self, x):
        ...
        h_ = jt.bmm(v,w_)     # b, c,hw (hw of q) h_[b,c,j] = sum_i v[b,c,i] w_[b,i,j]
